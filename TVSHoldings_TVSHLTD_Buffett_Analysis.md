# TVS Holdings Ltd (NSE: TVSHLTD) — Warren Buffett Style Analysis

**Date of Analysis:** March 26, 2026
**Current Market Price (CMP):** ₹14,228
**Market Capitalisation:** ₹28,787 Cr
**BSE Code:** 520056 | **NSE Symbol:** TVSHLTD

---

> **Sector Detection: STANDARD MODE (Investment / Holding Company)**
>
> TVS Holdings Ltd is a **Core Investment Company (CIC)** registered with the RBI (March 2024). While it is technically classified as an NBFC-CIC, its economic substance is that of a **holding company** — it does not originate loans, take deposits, or conduct banking operations itself. Its consolidated financials are driven overwhelmingly by its **50.26% stake in TVS Motor Company** and the latter's subsidiary **TVS Credit Services** (an NBFC). Therefore, the STANDARD MODE is applied to the consolidated entity, with supplementary NBFC commentary where TVS Credit's financials are material.

---

## 1. THE BUSINESS — Understanding the Economic Engine

> *"Never invest in a business you cannot understand."* — Warren Buffett

**TVS Holdings Ltd** (formerly Sundaram-Clayton Ltd) is the Venu Srinivasan family's apex holding entity for the TVS Group. The company underwent a major restructuring in 2022-23 — the die-casting manufacturing business was demerged to a separate entity (Sundaram-Clayton Ltd), and the residual entity was renamed TVS Holdings Ltd. It received CIC registration from the RBI in March 2024 and wound down its spare parts trading business (₹300-400 Cr revenue, negligible profitability) by October 2024.

**What does TVS Holdings actually own?**

| Holding | Entity | Nature | Approx. Value (as of late 2025) |
|---|---|---|---|
| **50.26%** | **TVS Motor Company Ltd** | India's 3rd-largest two-wheeler manufacturer | **~₹83,000 Cr** (at ₹3,476 CMP × 47.52 Cr shares × 50.26%) |
| Via TVSM | TVS Credit Services Ltd | NBFC — 2W, used cars, tractors, consumer durables financing; AUM ~₹28,645 Cr | Unlisted; estimated ₹8,000-12,000 Cr |
| Via TVSM | Norton Motorcycles (UK) | Premium motorcycle brand | Early stage |
| Via TVSM | Swiss E-Mobility Group (SEMG) | E-bikes in Europe | Growing |
| Direct | Home Credit India Finance | Acquired NBFC (regulatory merger pending with TVS Credit) | ~₹1,500-2,000 Cr estimated |
| Direct/Via TVSM | DriveX Mobility | Pre-owned 2W retail platform | Early stage |
| Direct | Other investments | Various group entities | Modest |

**The Buffett Lens: Is This Business Understandable?**

At its core, TVS Holdings is a **toll bridge on the Indian two-wheeler economy**. It owns the single largest stake in TVS Motor Company, which manufactures and sells motorcycles, scooters, mopeds, three-wheelers, and EVs — products that every Indian understands. Attached to this is a captive financing arm (TVS Credit) that finances purchases of these vehicles and adjacent products.

The business model is elegantly simple: **India's 1.4 billion people need affordable personal transportation, and TVS Motor sells it to them while TVS Credit finances it.** The holding company earns through dividend income, capital appreciation, and consolidated earnings flow-through.

However, the holding company structure adds a layer of **complexity and discount** that Buffett generally dislikes. Buffett prefers direct ownership of operating businesses. The CIC structure means TVS Holdings cannot diversify beyond investments — it exists purely as a family ownership vehicle.

> *"I want to be in businesses so good that even a fool can run them. Because sooner or later, one will."* — Buffett

TVS Motor itself passes this test — two-wheelers are a consumer franchise with strong brand loyalty, distribution moats, and high repeat purchase rates. The holding company is simply the wrapper.

---

## 2. INCOME STATEMENT ANALYSIS — Revenue & Profitability Trends

**Consolidated Figures in ₹ Crores**

| Metric | Mar 2018 | Mar 2019 | Mar 2020 | Mar 2021 | Mar 2022 | Mar 2023 | Mar 2024 | Mar 2025 | TTM (Dec 2025) |
|---|---|---|---|---|---|---|---|---|---|
| **Revenue** | 17,512 | 21,548 | 19,859 | 20,299 | 25,591 | 33,662 | 39,882 | 44,993 | **54,367** |
| Revenue Growth YoY | 30% | 23% | **-8%** | 2% | 26% | 32% | 18% | 13% | 26% |
| Operating Profit | 1,539 | 2,343 | 2,412 | 2,388 | 2,992 | 4,283 | 5,760 | 6,844 | **8,616** |
| OPM % | **9%** | 11% | 12% | 12% | 12% | 13% | 14% | 15% | **16%** |
| Other Income | 121 | 28 | -8 | 38 | 22 | 141 | 59 | 70 | 8 |
| Interest | 372 | 719 | 910 | 929 | 985 | 1,424 | 2,032 | 2,223 | 2,600 |
| Depreciation | 447 | 534 | 649 | 643 | 845 | 986 | 1,011 | 1,067 | 1,343 |
| PBT | 841 | 1,118 | **845** | 854 | 1,184 | 2,013 | 2,776 | 3,624 | **4,681** |
| Tax Rate | 25% | 33% | 26% | 31% | 30% | 34% | 36% | 34% | ~32% |
| **Net Profit** | 629 | 750 | **627** | 592 | 824 | 1,333 | 1,782 | 2,409 | **3,169** |
| Net Profit Growth | — | 19% | **-16%** | -6% | 39% | 62% | 34% | 35% | **42%** |
| EPS (₹) Diluted | 167 | 214 | **167** | 160 | 218 | 322 | 396 | **576** | **770** |
| EPS Growth | — | 28% | **-22%** | -4% | 36% | 48% | 23% | 45% | — |
| Shares Outstanding (Cr) | ~1.89 | ~1.89 | ~1.89 | ~1.89 | ~1.89 | ~2.02 | ~2.02 | ~2.02 | ~2.02 |
| Dividend Payout % | **9%** | 17% | 19% | 16% | 20% | 18% | **24%** | 16% | — |

> **Important Note — Consolidated Net Profit vs Attributable EPS:** The Net Profit figures above are **total consolidated NP** (including minority interest's share). Since TVS Holdings owns ~50.26% of TVS Motor Company (its dominant subsidiary), approximately half of consolidated earnings accrue to minority shareholders. The **EPS figures**, however, are computed by Screener on **profit attributable to the parent company's shareholders only** — hence EPS × Shares ≈ 48-54% of Consolidated NP across all years. This is standard accounting for holding companies and does not indicate an error. Wherever this distinction matters for per-share calculations (Owner Earnings, $1 Test, DCF), it is noted explicitly.
>
> **NP Growth vs EPS Growth divergence:** In FY23, NP Growth is 62% while EPS Growth is only 48%. This 14pp gap reflects two factors: (a) shares outstanding increased ~7% from ~1.89 Cr to ~2.02 Cr due to the FY22-23 demerger/restructuring, diluting EPS growth relative to NP growth; and (b) the attributable share of NP growth may differ from total consolidated NP growth.

**Trend Narrative:**

The story here is one of **powerful margin expansion accompanied by robust revenue growth**. Revenue has compounded at ~15% over 10 years and ~18% over 5 years. More impressively, operating margins have nearly doubled from 9% (FY18) to 16% (TTM), demonstrating significant operating leverage as TVS Motor premiumises its product mix (Apache, Jupiter 125, Ntorq, iQube EV) and scales internationally.

Net profit growth has been explosive — **26% CAGR over 5 years (per Screener), 37% CAGR over 3 years (per Screener).** This is earnings growth outpacing revenue growth by a wide margin, which is the hallmark of a business hitting operating leverage. (Note: Screener's "Compounded Profit Growth" uses **attributable profit** — i.e., profit after deducting minority interest — which is lower than the consolidated NP shown in the table above. Direct calculation on consolidated NP yields ~31% and ~43% respectively. The attributable-basis CAGRs from Screener are the more meaningful metric for equity shareholders.)

The interest cost has risen sharply (₹372 Cr in FY18 to ₹2,223 Cr in FY25) due to TVS Credit's scaling — this is the cost of financing a growing loan book, not corporate leverage in the traditional sense. This is a key nuance: most of the consolidated debt is TVS Credit's borrowings to fund its lending operations.

> *"The most important thing in evaluating a business is its ability to earn high returns on invested capital, and to continue doing so."* — Buffett

The trend here is unambiguously positive: rising revenues, expanding margins, accelerating earnings growth. This is a business firing on all cylinders.

---

## 3. BALANCE SHEET ANALYSIS — Financial Fortress Assessment

**Consolidated Figures in ₹ Crores**

| Metric | Mar 2018 | Mar 2019 | Mar 2020 | Mar 2021 | Mar 2022 | Mar 2023 | Mar 2024 | Mar 2025 | Sep 2025 |
|---|---|---|---|---|---|---|---|---|---|
| Equity Capital | 10 | 10 | 10 | 10 | 10 | 10 | 10 | 10 | 10 |
| Reserves | 2,141 | 2,464 | 2,453 | 2,894 | 5,069 | 3,231 | 2,830 | 4,677 | 5,503 |
| **Shareholders' Equity** | **2,151** | **2,474** | **2,463** | **2,904** | **5,079** | **3,241** | **2,840** | **4,687** | **5,513** |
| Borrowings | 7,593 | 10,022 | 12,314 | 12,853 | 16,593 | 25,217 | 26,237 | 32,489 | 34,480 |
| Other Liabilities | 5,016 | 5,674 | 5,835 | 7,551 | 8,662 | 10,665 | 15,453 | 16,718 | 18,282 |
| **Total Liabilities** | **14,761** | **18,170** | **20,612** | **23,309** | **30,334** | **39,123** | **44,530** | **53,895** | **58,276** |
| Fixed Assets | 3,627 | 4,030 | 4,460 | 4,850 | 6,339 | 6,931 | 5,630 | 7,519 | 9,066 |
| CWIP | 396 | 756 | 1,017 | 1,050 | 560 | 821 | 1,029 | 1,482 | 702 |
| Investments | 607 | 618 | 472 | 673 | 644 | 1,022 | 1,204 | 1,320 | 1,388 |
| Other Assets | 10,131 | 12,766 | 14,663 | 16,735 | 22,790 | 30,348 | 36,667 | 43,574 | 47,120 |
| **Total Assets** | **14,761** | **18,170** | **20,612** | **23,309** | **30,334** | **39,123** | **44,530** | **53,895** | **58,276** |
| D/E Ratio | 3.5x | 4.1x | 5.0x | 4.4x | 3.3x | 7.8x | 9.2x | 6.9x | 6.3x |
| Book Value/Share (₹) | 1,138 | 1,309 | 1,303 | 1,536 | 2,687 | 1,604 | 1,407 | 2,320 | **2,729** |

**Critical Context — Why the D/E Ratio Is Misleading:**

At first glance, a D/E ratio of 6-9x looks terrifying. But this is **not** a manufacturing company carrying that debt — the vast majority of the ₹32,489 Cr borrowings sits within **TVS Credit Services**, an NBFC whose entire business IS borrowing money to lend it. Just as deposits are the raw material for a bank, wholesale borrowings are the raw material for an NBFC. Stripping out TVS Credit's borrowings, TVS Motor itself has modest leverage, and TVS Holdings standalone has only ~₹944 Cr in gross debt (covered 87x by its TVSM stake).

The **Book Value Per Share** has been volatile due to the demerger adjustments (the die-casting business was carved out, which reduced reserves, and then the holding company restructuring added complexity). The current BVPS of ₹2,729 gives a P/B of 5.2x.

> *"We never want to count on the kindness of strangers in order to meet tomorrow's obligations."* — Buffett

**TVS Holdings standalone** is a financial fortress: ₹944 Cr debt against an investment in TVSM worth ~₹83,000 Cr — an asset coverage of ~87x. This is exceptional.

---

## 4. CASH FLOW ANALYSIS — The Lifeblood of Valuation

**Consolidated Figures in ₹ Crores**

| Metric | Mar 2018 | Mar 2019 | Mar 2020 | Mar 2021 | Mar 2022 | Mar 2023 | Mar 2024 | Mar 2025 |
|---|---|---|---|---|---|---|---|---|
| CFO | 324 | **-710** | 458 | 1,215 | **-1,548** | **-4,111** | -867 | **3,535** |
| Investing Activities | -1,379 | -1,324 | -1,055 | -979 | -1,705 | -1,439 | 961 | -2,857 |
| Financing Activities | 1,198 | 2,085 | 1,795 | 261 | 3,116 | 5,979 | 847 | 1,209 |
| **Net Cash Flow** | 143 | 51 | 1,199 | 496 | -137 | 429 | 941 | 1,886 |

**Why Consolidated FCF Is Misleading Here:**

The wildly volatile CFO (ranging from -₹4,111 Cr to +₹3,535 Cr) is driven almost entirely by **TVS Credit's loan book growth**. When TVS Credit grows its AUM aggressively (as it did in FY22-23, growing from ₹14,403 Cr to ₹20,602 Cr), every rupee lent shows up as a cash outflow. When growth moderates (as in FY25, AUM grew only 3%), cash flow normalises dramatically.

**This is the exact same dynamic described in the BANK/NBFC MODE guidance** — a growing lender will always look cash-negative, and this is NOT a sign of distress.

**Earnings Quality & Capital Generation (TVS Holdings Standalone + TVS Motor):**

| Metric | FY22 | FY23 | FY24 | FY25 |
|---|---|---|---|---|
| Consolidated Net Profit (₹ Cr) | 824 | 1,333 | 1,782 | 2,409 |
| Dividends Paid (approx.) | ~165 | ~240 | ~428 | ~386 |
| Retained Earnings | ~659 | ~1,093 | ~1,354 | ~2,023 |
| Retention Ratio | ~80% | ~82% | ~76% | ~84% |

The company retains 75-85% of earnings and reinvests them into growth (TVS Motor's capacity expansion, EV development, international growth; TVS Credit's AUM scaling). Shares increased by ~7% (from ~1.89 Cr to ~2.02 Cr) as part of the FY22-23 demerger/restructuring, but there have been **no equity dilutions since** — no QIPs, rights issues, or preferential allotments. This is exactly what Buffett wants: a business that funds growth through internal accruals.

---

## 5. CAPEX QUALITY ANALYSIS — Growth vs. Maintenance CapEx

| Metric | FY20 | FY21 | FY22 | FY23 | FY24 | FY25 |
|---|---|---|---|---|---|---|
| Revenue (₹ Cr) | 19,859 | 20,299 | 25,591 | 33,662 | 39,882 | 44,993 |
| Depreciation (₹ Cr) | 649 | 643 | 845 | 986 | 1,011 | 1,067 |
| Net Profit (₹ Cr) | 627 | 592 | 824 | 1,333 | 1,782 | 2,409 |
| CapEx (from investing CF, approx.) | ~1,055 | ~979 | ~1,705 | ~1,439 | ~1,000** | ~2,857 |
| CapEx as % of Revenue | 5.3% | 4.8% | 6.7% | 4.3% | 2.5% | 6.3% |

> *\*\*FY24 CapEx note: Net investing cash flow in FY24 was actually a positive ₹961 Cr (net inflow) because the die-casting business demerger resulted in large asset disposals that more than offset gross CapEx. The ~₹1,000 Cr figure shown here is an estimate of gross CapEx spending, not the net investing CF figure.*

**Owner Earnings Estimate (Buffett's Preferred Metric):**

TVS Motor is a manufacturing business that requires ongoing investment in plant, tooling, moulds, R&D, and EV development. A reasonable estimate of maintenance CapEx is approximately equal to depreciation (₹1,067 Cr in FY25).

| Owner Earnings (FY25) | ₹ Cr |
|---|---|
| Net Profit | 2,409 |
| Add: Depreciation & Amortisation | 1,067 |
| Less: Estimated Maintenance CapEx (~Depreciation) | -1,067 |
| **Owner Earnings** | **~2,409** |

In this case, Owner Earnings approximately equal Net Profit because the business is mature enough that maintenance CapEx roughly equals D&A. The growth CapEx above this level (EV plants, international expansion, Norton revival) is genuine investment in expanding the economic footprint.

> *"Owner earnings represent the relevant item for valuation purposes — not reported earnings, not cash flow."* — Buffett

---

## 6. RETURN ON CAPITAL — The Buffett Quality Test

| Metric | FY18 | FY19 | FY20 | FY21 | FY22 | FY23 | FY24 | FY25 |
|---|---|---|---|---|---|---|---|---|
| **ROE** | — | — | — | — | — | — | — | **30.2%** |
| ROE (10Y avg) | — | — | — | — | — | — | — | **18%** |
| ROE (5Y avg) | — | — | — | — | — | — | — | **19%** |
| ROE (3Y avg) | — | — | — | — | — | — | — | **23%** |
| **ROCE** | 16% | 15% | 12% | 11% | 11% | 12% | 15% | **15.2%** |

**Key Observations:**

TVS Holdings' consolidated ROE of **30.2%** (latest year) is exceptional. The 5-year average of 19% and 3-year average of 23% demonstrate a **rising trajectory** — this is not a one-time spike but a structural improvement driven by TVS Motor's premiumisation, margin expansion, and operating leverage.

ROCE at 15.2% is healthy and improving, though the high leverage from TVS Credit's borrowings affects the denominator.

**The DuPont decomposition reveals that the high ROE is driven by:**
1. **Improving net margins** (from ~3.6% in FY18 to ~5.4% in FY25 on consolidated revenue) — genuine pricing power and cost efficiency
2. **High asset turnover** for an auto manufacturer — efficient capital deployment
3. **Leverage** from TVS Credit's NBFC operations — which is the nature of the business, not financial engineering

> *"The best business to own is one that over an extended period can employ large amounts of incremental capital at very high rates of return."* — Buffett

TVS Motor is delivering this: it is reinvesting retained earnings at 25-30% ROE through capacity expansion, EV development, and international growth. This is the Buffett sweet spot.

---

## 7. COMPETITIVE POSITION & MOAT ANALYSIS

**Moat Type: Consumer Franchise + Distribution + Captive Finance (Moderate-to-Strong)**

**TVS Motor's Competitive Position:**

TVS Motor is India's **3rd-largest two-wheeler manufacturer** (after Hero MotoCorp and Honda 2W), with FY25 total sales of ~51+ lakh 2Ws and 3Ws (domestic + international). In the domestic 2W market (~18M units), TVS commands **~20-21% market share** and has been the **fastest-growing major player**, gaining share consistently.

| Company | Market Share (approx.) | Moat Source |
|---|---|---|
| Hero MotoCorp | ~28% | Scale, rural distribution, brand |
| Honda 2W India | ~25% | Technology, brand, scooter dominance |
| **TVS Motor** | **~20-21%** | **Product innovation, brand premiumisation, EV leadership, captive finance** |
| Bajaj Auto | ~16% | Export focus, Pulsar brand, KTM alliance |
| Royal Enfield (Eicher) | ~8% | Premium niche, cult brand |

**Moat Assessment:**

1. **Brand Power:** TVS has built strong sub-brands — Apache (performance motorcycles), Jupiter (scooters), Ntorq (youth scooters), iQube (EVs), and Raider. These are **aspiration brands** that command pricing power, especially vs. Hero and Honda's mass-market positioning.

2. **Distribution:** 46,500+ customer touchpoints across India, plus presence in 60+ countries. International business is growing 35%+ and now contributes meaningfully. Africa is a major growth market (50% of international revenue).

3. **Captive Finance (TVS Credit):** This is a **structural moat** — TVS Credit finances ~25% of TVS Motor's domestic sales. This creates a virtuous cycle: easier financing → more sales → larger loan book → more fee income → more data for underwriting. Competitors without captive NBFCs (Royal Enfield, Bajaj) are at a disadvantage.

4. **EV Leadership:** TVS iQube is among the top-selling electric scooters in India. The company has invested heavily in EV R&D and manufacturing. With the acquisition of SEMG (Swiss E-Mobility Group), it has a European e-bike presence. This is a **call option on the EV transition** in India.

5. **Technology & Innovation:** TVS Racing heritage, partnership with BMW Motorrad (Apache RR 310 derived from collaboration), acquisition of Norton Motorcycles (UK premium brand), and Engines Engineering (Italian design house) demonstrate commitment to moving up the value chain.

**Is the Moat Enduring?**

The two-wheeler market in India is a **proven, enduring franchise**. Buffett would appreciate that the fundamental product — affordable personal transportation for a billion people — is unlikely to be disrupted. India's two-wheeler penetration is still low vs. ASEAN peers, and the shift from public transport to personal mobility is secular.

The EV transition is the biggest change risk, but TVS is ahead of legacy competitors (Hero, Honda) in EV adoption. The threat is more from pure-EV startups (Ola Electric, Ather) than from traditional competitors.

> *"We're looking for the absence of change to protect ways that are already making a lot of money."* — Buffett

The core two-wheeler business is remarkably stable and slow-changing. People will buy 2Ws in India for decades. The propulsion shift (ICE to EV) is the main change vector, and TVS is well-positioned to navigate it.

---

## 8. SCALABILITY CHECK — Can This Business Scale and Is It Scaling?

### Part A: Unit Economics of Scaling

**Profit Scalability — Does Growth Translate Into Disproportionate Profit Growth?**

| Year | Revenue (₹ Cr) | Rev Growth | Op Profit (₹ Cr) | Op Profit Growth | Net Profit (₹ Cr) | NP Growth | Incremental Op Margin |
|---|---|---|---|---|---|---|---|
| FY20 | 19,859 | -8% | 2,412 | 3% | 627 | -16% | — |
| FY21 | 20,299 | 2% | 2,388 | -1% | 592 | -6% | Negative |
| FY22 | 25,591 | 26% | 2,992 | 25% | 824 | 39% | 11.4% |
| FY23 | 33,662 | 32% | 4,283 | 43% | 1,333 | 62% | **16.0%** |
| FY24 | 39,882 | 18% | 5,760 | 34% | 1,782 | 34% | **23.7%** |
| FY25 | 44,993 | 13% | 6,844 | 19% | 2,409 | 35% | **21.2%** |
| TTM | 54,367 | 26% | 8,616 | — | 3,169 | 42% | — |

**The incremental operating margin has been 16-24%, significantly above the current operating margin of 15-16%.** This is the hallmark of an **elite scaler** — each new rupee of revenue is more profitable than the last. TVS Motor is past the fixed-cost absorption inflection point and is now riding operating leverage beautifully.

### Part B: Scaling Runway

**Total Addressable Market vs. Current Penetration:**

India's two-wheeler market is ~18-20 million units annually (domestic) and growing. TVS sold ~51+ lakh (5.1 million) total 2W+3W units in FY25 (including international), with domestic 2W sales of ~37 lakh (3.7 million) — implying a **domestic 2W market share of ~20-21%**. International markets (60+ countries, especially Africa and ASEAN) add another massive runway. The EV transition opens an entirely new segment. TVS Credit's AUM of ₹27,000 Cr is tiny vs. the addressable opportunity in vehicle financing (Hero MotoCorp alone sells 5+ million units that need financing).

**S-Curve Position: GROWTH PHASE (approaching mid-scaling)**

TVS Motor is firmly in the **Growth Phase** — the model is proven, unit economics are validated, margins are expanding with scale, and significant runway remains domestically and internationally. It is past the inflection point but nowhere near saturation.

### Part C: Scaling Verdict

| Dimension | Score (1-10) | Evidence |
|---|---|---|
| Revenue scalability | 8 | Strong brand allowing geographic expansion; international growing 35%+ |
| Profit scalability | **9** | Incremental margins 16-24% vs. current 15-16%; operating leverage kicking in |
| Capital scalability | 7 | Manufacturing requires CapEx, but premiumisation reduces unit volumes needed per rupee of revenue |
| Scaling runway | **9** | Low Indian 2W penetration, vast international opportunity, EV transition |
| S-Curve position | 8 | Growth phase with clear operating leverage; sweet spot for compounding |
| **Overall Scalability Score** | **8.2/10** | **Strong Scaler — GEICO / American Express analogy** |

---

## 9. MANAGEMENT QUALITY & BUFFETT'S $1 TEST

**Management:** Venu Srinivasan (Chairman) and Sudarshan Venu (MD, TVS Motor — next generation). The Srinivasan family has a sterling reputation in Indian industry for integrity, long-term thinking, and quality obsession. Venu Srinivasan is a Deming Prize recipient and transformed TVS Motor from a moped maker into a global two-wheeler powerhouse.

**Buffett's $1 Test:**

Over the 5-year period FY20-FY25:
- Total retained earnings (consolidated, approx.): ~₹6,100 Cr
- Market cap increase: from ~₹4,000 Cr (FY20, CMP ~₹2,100) to ~₹28,787 Cr = **~₹24,800 Cr increase**
- **For every ₹1 retained, ~₹4.05 of market value was created.** This passes the $1 test spectacularly.

> *Note: The ₹6,100 Cr retained earnings figure uses consolidated NP (including minority interest's ~50% share). On an attributable basis (profit accruing to TVS Holdings shareholders only), retained earnings were ~₹2,960 Cr, giving a $1 test ratio of ~₹8.38 per ₹1 retained — nearly double, and even more impressive. The test passes emphatically on either basis.*

**Promoter Holding:** 74.45% — extremely high skin in the game. The VS Trust (Venu Srinivasan family) alone holds 66.55%.

**Capital Allocation:** No dilutive equity raises since the restructuring. Sensible dividend payout (16-24%). Retained earnings reinvested at 25-30% ROE. Acquisitions (Norton, SEMG, Home Credit India) are strategic and bolt-on rather than empire-building. This is textbook Buffett-quality capital allocation.

> *"When a management team with a reputation for brilliance tackles a business with a reputation for bad economics, it is the reputation of the business that remains intact."* — Buffett

Here we have the opposite: excellent management running an excellent business. The Srinivasan family has compounded value for shareholders over decades.

---

## 10. OPERATING METRICS — Industry-Specific KPIs

**TVS Motor Company — Two-Wheeler & Three-Wheeler Sales (Lakh Units):**

| Period | Domestic 2W | International 2W | Total 2W+3W | YoY Growth |
|---|---|---|---|---|
| FY22 | ~28 | ~7 | ~35 | ~25% |
| FY23 | ~32 | ~8 | ~40 | ~15% |
| FY24 | ~34 | ~11 | ~45+ | ~12% |
| FY25 | ~37 | ~14 | ~51+ | ~13% |
| Q3 FY26 (Dec 2025) | — | — | — | 30% (qtr growth) |
| Feb 2026 (monthly) | 3.66 L domestic | 1.58 L intl | 5.29 L total | **31% YoY** |

**TVS Credit Services — Key Metrics:**

| Metric | FY22 | FY23 | FY24 | FY25 | H1 FY26 |
|---|---|---|---|---|---|
| AUM (₹ Cr) | 14,403 | 20,602 | 25,900 | 26,647 | 28,645 |
| AUM Growth | — | **43%*** | 26% | **3%** | ~7.5% (H1) |
| Disbursements (₹ Cr) | ~17,000 | ~21,652 | 25,018 | 26,301 | — |
| Total Income (₹ Cr) | — | 4,160 | 5,795 | 6,630 | ~3,400 |
| PAT (₹ Cr) | — | 389 | ~500 | ~600 | — |
| GNPA % | ~4.5% | ~2.7% | 2.81% | 2.89% | ~3.0% |

**Key Takeaway:** TVS Motor is firing on all cylinders with 30%+ growth in recent months. TVS Credit went through a period of calibrated growth in FY25 (only 3% AUM growth) to focus on asset quality, but is now resuming growth (12% disbursement growth in Q1 FY26). The management discipline to slow down when asset quality needed attention is a positive signal.

> *\*FY23 AUM Growth: Direct calculation from the table (20,602/14,403 − 1) yields ~43%. TVS Credit's own press release states "48% growth from Mar'22" — the difference may arise from a slightly different base AUM definition (e.g., ₹13,928 Cr on a gross loan receivable basis vs ₹14,403 Cr on a managed AUM basis). H1 FY26 growth of ~7.5% is the sequential 6-month increase (Mar 2025 → Sep 2025). The CARE Ratings report states AUM "increased by 5% in H1FY26" which likely uses a different AUM base (₹27,179 Cr gross loan receivables vs ₹26,647 Cr managed AUM).*

---

## 11. VALUATION ANALYSIS — The Price You Pay Determines Your Return

**This is a Holding Company — SOTP (Sum of the Parts) is the primary valuation method.**

| Component | Valuation Basis | Value (₹ Cr) | Per Share (₹) |
|---|---|---|---|
| **50.26% stake in TVS Motor** | Market value at ₹3,476/share × 47.52 Cr shares × 50.26% | **~82,900** | **~41,040** |
| TVS Credit Services (via TVSM) | 2.5-3x book value (NBFC with ~₹3,500 Cr net worth, 50.26% share) | ~4,400-5,300 | ~2,180-2,620 |
| Home Credit India (direct holding) | Estimated at ~1x book | ~1,500-2,000 | ~740-990 |
| Other subsidiaries & investments | Nominal value | ~500-1,000 | ~250-500 |
| **Gross Asset Value** | | **~89,300-91,200** | **~44,210-45,150** |
| Less: TVS Holdings standalone debt | ₹944 Cr | -944 | -467 |
| **Net Asset Value (NAV)** | | **~88,350-90,250** | **~43,740-44,680** |
| **Holding Company Discount (typical 40-60%)** | | | |
| NAV at 40% discount | | ~53,000-54,150 | **~26,240-26,800** |
| NAV at 50% discount | | ~44,175-45,125 | **~21,870-22,340** |
| NAV at 60% discount | | ~35,340-36,100 | **~17,500-17,870** |

**Current Price of ₹14,228 implies a HOLDCO DISCOUNT of ~67-68% to NAV.**

**Historical Valuation Multiples:**

| Metric | FY20 | FY21 | FY22 | FY23 | FY24 | FY25 | Current (Screener) |
|---|---|---|---|---|---|---|---|
| PE Ratio | — | 37.6x | 16.3x | **11.5x** | **22.9x** | 15.6x | **18.4x** |
| P/B Ratio | — | — | — | — | — | — | **5.2x** |
| Historical Median PE (10Y) | — | — | — | — | — | — | **~14.5x** |

*Note on PE: Screener reports a "Stock P/E" of 18.4x which uses its own methodology (likely based on consolidated attributable earnings, adjusted for minority interests). On the reported FY25 EPS of ₹576.33, the PE would be 24.7x. On the TTM EPS of ₹769.58 (sum of last 4 quarters), the PE is 18.5x — closely matching Screener's figure. This confirms the market is pricing in the TTM earnings trajectory, not just the FY25 annual figure.*

The consolidated PE on FY25 reported EPS of ₹576 is ~24.7x, while on the trailing twelve months (TTM) EPS of ~₹770, the PE drops to ~18.5x. This is optically cheap for a business growing earnings at 35%+ and with an ROE of 30%. The Screener-reported PE of 18.4x uses a different methodology but confirms the stock trades at a moderate valuation relative to its growth trajectory.

---

## 12. INTRINSIC VALUE & MARGIN OF SAFETY

### Method 1: SOTP-Based Intrinsic Value (Primary Method for Holding Companies)

This is the most appropriate method. The intrinsic value depends critically on two variables: (a) what TVS Motor is worth, and (b) what holding company discount the market applies.

**Conservative Scenario:**
- TVS Motor at current market price (no premium)
- TVS Credit at 2x book
- 55% holdco discount
- **Intrinsic Value: ~₹19,500/share** (37% upside)

**Base Scenario:**
- TVS Motor at current market price
- TVS Credit at 2.5x book
- 50% holdco discount
- **Intrinsic Value: ~₹22,000/share** (55% upside)

**Optimistic Scenario:**
- TVS Motor 15% above current (reflecting growth trajectory) — this premium already embeds an enhanced valuation for TVS Credit (a consolidated subsidiary of TVSM), so TVS Credit is **not** added separately to avoid double-counting
- Home Credit India at 1.5x book + other assets
- 40% holdco discount
- **Intrinsic Value: ~₹28,000/share** (97% upside)

### Method 2: Consolidated Earnings-Based DCF

Using Owner Earnings of ~₹2,400 Cr (FY25) as starting point:

> **Methodology Note:** This DCF uses **consolidated** Owner Earnings (₹2,409 Cr), which includes minority shareholders' ~50% share of TVS Motor's earnings. The resulting per-share IV therefore represents value under the assumption that consolidated cash flows accrue proportionally to the holding company — which is directionally correct for a controlling shareholder (50.26% stake with board control) but overstates the strict attributable cash flow. The **SOTP method above is the primary valuation approach** and does not have this issue. The DCF is presented as a cross-check; if run on attributable Owner Earnings (~₹1,163 Cr), the per-share IVs would be roughly half the stated figures, but the growth rates and discount rates would also need recalibration for consistency. The averaged summary below blends both methods.

| Scenario | Growth (Yr 1-5) | Growth (Yr 6-10) | Terminal Growth | Discount Rate | Intrinsic Value/Share | Upside from CMP |
|---|---|---|---|---|---|---|
| **Conservative** | 15% | 10% | 5% | 12% | **₹17,800** | 25% |
| **Base** | 20% | 12% | 5% | 11% | **₹25,400** | 78% |
| **Optimistic** | 25% | 15% | 5% | 10% | **₹38,500** | 171% |

**Summary:**

| Scenario | SOTP Value | DCF Value | Avg. Intrinsic Value | Upside from CMP ₹14,228 | Margin of Safety (IV-CMP)/IV |
|---|---|---|---|---|---|
| Conservative | ₹19,500 | ₹17,800 | **₹18,650** | **31%** | **24%** |
| Base | ₹22,000 | ₹25,400 | **₹23,700** | **67%** | **40%** |
| Optimistic | ₹28,000 | ₹38,500 | **₹33,250** | **134%** | **57%** |

---

## 13. REVERSE DCF — What Growth Is the Market Pricing In?

**Inputs:**
- CMP: ₹14,228
- TTM EPS (normalised): ~₹770 (using trailing 12-month data through Dec 2025)
- Current PE on TTM: ~18.5x
- Historical Median PE (10Y): ~14.5x

**Implied EPS CAGR Needed to Achieve Target Returns:**

Formula: Required Future Price = CMP × (1 + Hurdle)^n; Required Future EPS = Required Future Price / Terminal PE; Implied EPS CAGR = (Required Future EPS / Current EPS)^(1/n) - 1

| Holding Period | Terminal PE 15x | Terminal PE 20x | Terminal PE 25x |
|---|---|---|---|
| **10% Return, 5 Yr** | 14.7% CAGR | 8.3% CAGR | 3.5% CAGR |
| **10% Return, 10 Yr** | 12.3% CAGR | 9.1% CAGR | 6.7% CAGR |
| **15% Return, 5 Yr** | 19.9% CAGR | 13.2% CAGR | 8.3% CAGR |
| **15% Return, 10 Yr** | 17.4% CAGR | 14.1% CAGR | 11.6% CAGR |
| **20% Return, 5 Yr** | 25.1% CAGR | 18.1% CAGR | 13.0% CAGR |
| **20% Return, 10 Yr** | 22.5% CAGR | 19.1% CAGR | 16.4% CAGR |

**Reality Check:**

| Benchmark | EPS CAGR |
|---|---|
| Historical 5Y EPS CAGR (FY20-FY25) | **~28%** |
| Historical 3Y EPS CAGR (FY22-FY25) | **~38%** |
| India nominal GDP growth | 10-12% |
| Screener-reported 5Y Profit CAGR | **26%** |

**Traffic Light Assessment:**

At Terminal PE = 20x (reasonable for a quality holding company):
- To earn 10% return over 10 years, the company needs **9.1% EPS CAGR** — this is below India's nominal GDP growth. **🟢 GREEN — Extremely easy to achieve.**
- To earn 15% return over 10 years, the company needs **14.1% EPS CAGR** — roughly half of what the company has delivered historically. **🟢 GREEN — Easily achievable.**
- To earn 20% return over 10 years, the company needs **19.1% EPS CAGR** — below the historical CAGR. **🟢 GREEN — Achievable with continued execution.**

Even at Terminal PE = 15x (a very conservative de-rating assumption):
- To earn 15% return over 10 years needs **17.4% EPS CAGR** — still below historical delivery. **🟢 GREEN.**
- To earn 20% return over 10 years needs **22.5% EPS CAGR** — roughly in line with history. **🟡 YELLOW — Requires sustained execution at near-historical rates.**

> "At the current price of ₹14,228, to earn a 15% annualised return over 10 years assuming the market values the business at 20x PE at exit, the company needs to grow EPS at just 14.1% CAGR. The company has historically grown EPS at 26-28% CAGR over 5 years. This implies **the market is pricing in very conservative growth** — roughly half of what the business has actually delivered."

This is a **🟢 GREEN** signal — the market is NOT demanding heroic assumptions. In fact, the bar the market is implicitly setting is unusually low for a business of this quality.

---

## 14. ROE QUALITY CHECK & RETURN EXPECTATION FRAMEWORK

### Part A: DuPont Decomposition

| Year | Net Profit Margin | Asset Turnover | Equity Multiplier | ROE (DuPont Product) |
|---|---|---|---|---|
| FY22 | 3.2% | 0.95x | ~6.7x | ~20.6%* |
| FY23 | 4.0% | 0.97x | ~10.0x | ~38.5%* |
| FY24 | 4.5% | 0.95x | ~14.6x | ~62.5%* |
| FY25 | 5.4% | 0.91x | ~13.1x | **~64.0%*** |

*Note: The DuPont decomposition uses **consolidated Net Profit** (₹2,409 Cr for FY25) in the numerator — which includes minority shareholders' ~50% share of TVS Motor's earnings. Screener's reported ROE of 30.2% uses **attributable Net Profit** (~₹1,163 Cr) — this is the primary reason the DuPont product (~64%) diverges from Screener's ROE (~30%). The equity base is similar in both calculations; it is the profit numerator that differs by ~2x. Additionally, the equity multiplier is volatile due to demerger-related swings in consolidated equity (the die-casting business carve-out drastically altered the equity base between FY22 and FY24). The directional insight remains valid: ROE is driven by genuinely improving margins, not financial engineering.*

**The key insight:** ROE is being driven by **improving net margins** (3.2% → 5.4%) which is genuine business improvement, combined with inherently high leverage from TVS Credit's NBFC operations. This is NOT financial engineering — it is the natural structure of a conglomerate that includes a lending arm.

### Part B: P/B Scenario Analysis

**Inputs:**
- Current BVPS: ₹2,729
- CMP: ₹14,228
- Current P/B: 5.2x
- Sustainable ROE: ~25% (normalised, below peak)
- Retention Ratio: ~80%
- BV Compounding Rate: 25% × 80% = **20%**

**Projected BVPS:**

| Year | Projected BVPS (₹) |
|---|---|
| Current | 2,729 |
| Year 5 | 6,792 |
| Year 7 | 9,783 |
| Year 10 | 16,889 |

**Expected Annual Return (CAGR) at CMP ₹14,228:**

| Exit P/B | Rationale | 5-Yr CAGR | 7-Yr CAGR | 10-Yr CAGR |
|---|---|---|---|---|
| 5.2x (stays) | Multiple sustains | **19.9%** | **20.0%** | **20.0%** |
| 4.0x | Mild compression | **13.8%** | **15.5%** | **16.9%** |
| 3.0x | Significant compression | **7.4%** | **10.9%** | **13.5%** |
| 2.5x | Reverts to fair value | **3.6%** | **8.0%** | **11.5%** |
| 2.0x | Deep compression | **-0.9%** | **4.6%** | **9.0%** |

**Key Insight:** Even if the P/B compresses from 5.2x to 3.0x over 10 years (a massive de-rating), the investor still earns ~13.5% CAGR because book value is compounding at 20%. This is the power of high ROE.

**Verdict: 🟢 GREEN — The 10-year return exceeds the 10% hurdle rate even if P/B compresses from 5.2x to ~2.5x (less than half).**

> *Note: The return CAGRs above are price-appreciation only and exclude dividend income (~0.5-0.7% yield). Total returns would be modestly higher.*

---

## 15. BUY ZONE DEFINITION

Given the holding company nature, buy zones are defined using multiple frameworks:

| Zone | PE Range (Consol.) | Approx. Price Range | Holdco Discount (indicative) | Rationale |
|---|---|---|---|---|
| **Deep Value / Strong Buy** | <10x | Below ₹7,700 | >75% | Exceptional — market ignoring the franchise completely |
| **Value / Buy** | 10-15x | ₹7,700 - ₹11,550 | 65-75% | Attractive — significant margin of safety |
| **Fair Value / Hold** | 15-20x | ₹11,550 - ₹15,400 | 55-65% | Fairly priced — hold for compounding |
| **Fully Valued / Trim** | 20-25x | ₹15,400 - ₹19,250 | 45-55% | Above fair value — limited near-term upside |
| **Overvalued / Sell** | >25x | Above ₹19,250 | <45% | Priced for perfection |

> *Note: The holdco discount ranges are indicative/aspirational, not mechanically derived from the PE zones. They represent the approximate discount at which a long-term investor would find each zone attractive. The actual holdco discount at any price depends on the prevailing NAV, which fluctuates with TVSM's market price. At current NAV of ~₹89,000 Cr, the precise calculated discounts are ~10pp higher than the indicative ranges shown (e.g., Fair Value zone corresponds to ~65-74% actual discount rather than 55-65%).*

**Current Price ₹14,228 falls in the FAIR VALUE / HOLD zone** (PE ~18.5x on TTM, holdco discount ~67%).

**If You Already Own the Stock — Buffett's Recommendation:**

| Your Entry Zone | Entry Price (approx.) | Current Position | Buffett's Advice |
|---|---|---|---|
| Deep Value (<₹7,700) | ₹5,000-7,700 | **+85% to +185% profit** | HOLD firmly. The business is compounding at 25%+ ROE. Let it ride. Trim only if it enters Overvalued zone. |
| Value (₹7,700-11,550) | ₹8,000-11,500 | **+24% to +78% profit** | HOLD. The thesis is intact and strengthening. Accumulate on any 15-20% dips back into this zone. |
| Fair Value (₹11,550-15,400) | ₹12,000-15,000 | **-5% to +19%** | HOLD. You bought at fair value — returns will track ROE/BV compounding from here (~15-20% CAGR). Add only on significant dips. |
| Fully Valued (₹15,400-19,250) | ₹16,000+ | **-11% loss** | HOLD but don't add. If thesis stays strong, compounding will bail you out within 1-2 years. Consider adding only below ₹12,000. |

> *"Our favourite holding period is forever."* — Buffett

For a small capital base, the optimal strategy is to accumulate aggressively in the Deep Value and Value zones, hold through Fair Value, and resist the urge to sell a 25%+ ROE compounder just because it's at fair value. Let the compounding do the work.

---

## 16. WOULD BUFFETT HIMSELF BUY THIS STOCK?

**Full Buffett Checklist:**

| Criterion | Assessment | Pass? |
|---|---|---|
| Circle of Competence — Is the business understandable? | Two-wheelers for India + captive NBFC. Buffett bought GEICO (auto insurance). He understands this. | ✅ |
| Enduring Moat | Consumer franchise + distribution + captive finance. Moat widening through premiumisation & EV. | ✅ |
| Trustworthy & Rational Management | Srinivasan family — stellar reputation, 74.45% ownership, no dilutive equity raises post-restructuring, Deming Prize. | ✅ |
| Resistant to Technological Disruption | 2Ws are enduring; TVS is leading the EV transition rather than being disrupted by it. | ✅ |
| Can sustain economics for 10+ years? | India's need for affordable mobility is a multi-decade secular trend. | ✅ |
| High ROE with reinvestment opportunity | 30% ROE, 80% retention, reinvesting at attractive returns. | ✅ |
| Price significantly below intrinsic value? | At ₹14,228, 67% holdco discount to NAV; base intrinsic value ~₹23,700 (67% upside from CMP, 40% margin of safety). | ✅ (Moderate) |
| Simple enough for napkin math? | "Owns half of India's 3rd-largest 2W maker, growing 25%+, at ~18.5x TTM earnings." Yes. | ✅ |

**All 8 criteria pass.**

**However, Buffett's hesitation would be:** He generally dislikes holding companies because you're buying a wrapper, not the operating business directly. He'd prefer to buy TVS Motor directly. The holdco discount is a permanent feature that caps returns relative to the underlying asset. The question is whether the discount narrows or stays.

**Verdict:** This is a **True Buffett Business wrapped in a structure Buffett dislikes.** If he were working with a small capital base (where he could buy the less liquid TVS Holdings rather than the highly liquid TVS Motor), he would likely find this attractive because:

1. The **implied valuation of TVS Motor through TVS Holdings is much cheaper** than buying TVS Motor directly (effectively buying TVSM at 67% discount to market price)
2. You get TVS Credit Services for free at these prices
3. The promoter's economic interest is 100% aligned through the holding company

> *"Be greedy when others are fearful."* — Buffett

The market's fear here is the holding company discount and the complexity premium. But the underlying economics are pristine.

---

## 17. RISK MATRIX

| Risk | Severity (1-10) | Probability (1-10) | Mitigation |
|---|---|---|---|
| **Holding company discount persists/widens** | 7 | 8 | Structural risk; can be mitigated if RBI allows holding company restructuring or if TVS Credit IPO unlocks value |
| **TVS Motor growth slowdown** | 8 | 3 | Domestic 2W market is growing; international expanding 35%+; EV adds new growth vector |
| **TVS Credit asset quality deterioration** | 7 | 4 | GNPA at 2.9% is manageable; management showed discipline by slowing growth in FY25 |
| **EV transition disruption** | 6 | 3 | TVS iQube is a top seller; company is investing heavily; Norton/SEMG provide global EV exposure |
| **Regulatory risk (CIC norms, NBFC regulations)** | 5 | 3 | CIC registration provides regulatory clarity; CARE AA+ rating confirms strong standing |
| **Commodity cost inflation (steel, aluminium)** | 5 | 5 | Partially passed through pricing; premiumisation reduces sensitivity |
| **Competition from new EV entrants (Ola, Ather)** | 5 | 5 | TVS has superior distribution, brand trust, and service network vs. startups |
| **Key man risk (Venu Srinivasan)** | 4 | 2 | Sudarshan Venu (next gen) is already MD of TVS Motor; succession underway |
| **Currency risk (international operations)** | 4 | 5 | Diversified across 60+ countries; natural hedge as exports grow |
| **Home Credit India integration risk** | 4 | 4 | Regulatory merger with TVS Credit pending; execution risk exists |

---

## 18. TURNAROUND PROBABILITY

**Not applicable.** TVS Holdings is NOT a turnaround story — it is a **growth compounder** firing on all cylinders. Revenue growing ~18% CAGR (5Y), attributable profit growing ~26% CAGR (5Y per Screener; ~28% on EPS basis), margins expanding, ROE at 30%, and the underlying businesses are gaining market share.

---

## 19. FINAL SCORECARD & SUMMARY

| Factor | Score (1-10) | Commentary |
|---|---|---|
| **Business Quality** | **9** | India's 3rd-largest 2W maker + captive NBFC. Consumer franchise with pricing power. |
| **Moat Durability** | **8** | Brand, distribution, captive finance, EV leadership. Moat widening, not narrowing. |
| **Financial Health** | **7** | Standalone fortress (87x debt cover). Consolidated leverage from NBFC is structural, not concerning. |
| **Management Quality** | **9** | Srinivasan family — impeccable reputation, 74.45% ownership, no dilutive equity raises post-restructuring, Deming Prize winner. |
| **Earnings Growth** | **9** | ~28% EPS CAGR (5Y), ~38% (3Y). Screener reports 26%/37% on attributable profit basis. Margin expansion ongoing. |
| **Scalability** | **8** | Elite operating leverage; massive domestic + international runway. |
| **Valuation** | **7** | ~18.5x TTM PE for 25%+ grower is reasonable. But P/B of 5.2x and holdco discount dynamics add uncertainty. |
| **Margin of Safety** | **7** | ~67% holdco discount to NAV provides significant embedded safety. Base intrinsic value ~67% above CMP (40% MoS). |
| **Risk Profile** | **7** | Main risk is structural holdco discount, not business fundamentals. |
| **Overall Score** | **7.9/10** | |

**Buffett Verdict:**

TVS Holdings is the family silver of Indian two-wheeler manufacturing, wrapped in a holding company structure at a 67% discount to its sum-of-parts value. The underlying business — TVS Motor Company — is a world-class consumer franchise growing at 25%+ with a 30% ROE, led by an owner-operator family with impeccable integrity. The captive NBFC (TVS Credit) adds a financial services compounding engine that the market is getting essentially for free at current prices.

At ₹14,228, the stock sits in the **Fair Value zone** — not dirt cheap, but offering a compelling risk-reward for a patient investor. The holdco discount provides a structural margin of safety, and if it narrows even modestly (from 67% to 50%), that alone generates 50%+ returns on top of the underlying earnings compounding.

Buffett would say: *"This is a wonderful business at a fair price. If I were working with a small capital base, I would buy this and hold it for a decade, knowing that the underlying economics will compound my wealth at 15-20% annually regardless of what Mr. Market does in the short term."*

**Recommendation: BUY for long-term compounding. Accumulate aggressively on any dips to the ₹10,000-12,000 range (Value zone). Hold existing positions and let the compounding work.**

---

## 20. TEST SIMPLICITY / NO BRAINER NATURE

**Can this be explained to a 10-year-old?**

*"There's a company that makes motorcycles and scooters that millions of Indians ride every day. It's growing bigger every year because more people want their own vehicle. The company that owns half of this motorcycle maker is selling for much less than what that half is actually worth. So you're buying something for ₹14,000 that has ₹44,000 worth of stuff inside it."*

**Does the math work on a napkin?**

- TVS Motor is worth ~₹1,66,000 Cr in the market
- TVS Holdings owns 50.26% of it = ~₹83,000 Cr
- TVS Holdings' market cap = ₹28,787 Cr
- **You're paying ₹29,000 Cr for ₹83,000 Cr worth of TVS Motor alone — that's 35 paise for every rupee of TVS Motor value. And you get TVS Credit (worth ₹8,000-12,000 Cr) and other assets essentially for free.**
- Even if the discount never narrows, the underlying business compounds at 20%+ ROE

**Do you need Excel?** No. The napkin math works. This is approximately right — and as Buffett says, it's better to be approximately right than precisely wrong.

> *"If you need to use a calculator or a computer to figure out whether it's a good buy, you shouldn't buy it."* — Buffett

The TVS Holdings thesis passes the simplicity test. Buy India's best two-wheeler franchise at a 65%+ discount to its market value, backed by a family with 74.45% ownership and a century of trust. Hold for a decade. Let compounding do the rest.

---

*Disclaimer: This analysis is for educational purposes only and does not constitute investment advice. All investments carry risk. The author is not a registered financial advisor. Please consult a qualified financial professional before making any investment decisions.*